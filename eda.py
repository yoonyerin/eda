...
def labeling_df(attr):
    ann_file=pd.read_csv(attr)
     
    ann_file=ann_file.replace(-1, "0")
    ann_file=ann_file.replace(1, "1")
    
    ann_file["onehot"]=ann_file["Oval_Face"]+ann_file["Young"]+ann_file["Big_Nose"]+ann_file["Big_Lips"]+ann_file["High_Cheekbones"]+ann_file["Arched_Eyebrows"]
    print(ann_file.head())
    ann_file.to_csv(f"{group_folder}/v2_group.csv", index=False)
        
#labeling_df(ATTRIBUTE_FILE)    
...
def service_dataset_distribution(attr, mode="include", chosen=[], limit=100, fix=0):
    ...
    if mode=="exclued":
        ...
        # ann_file["hair_color"]=ann_file["Black_Hair"]+ann_file["Blond_Hair"]\
        #     +ann_file["Gray_Hair"]+ann_file["Brown_Hair"]
        # for i, vect in enumerate(onehot):
        #     ann_file["hair_color"]=ann_file["hair_color"].replace(vect, i)
        # # for i in 
    ...
    print(f'{fix}: {selected_vector["count"].sum()}')
    
    selected_vector.to_csv(f"{group_folder}/{fix}_group_number.csv", index=False)
...
chosen=["Attractive", "Oval_Face", "Pale_Skin", "Young",                                                                            
           "Narrow_Eyes", "Arched_Eyebrows", "Big_Lips", "High_Cheekbones",
           'Black_Hair', 'Blond_Hair', 'Brown_Hair', 'Gray_Hair',] 
service_dataset_distribution(ATTRIBUTE_FILE, "all", chosen, 100, "neoda_simple")    
...
def skin_classification():
    _type=["sensitive", "none-sensitive"]
    plt.bar( [2, 4], [65, 77], 0.4, label = _type, color=["blue", "navy"])
    # plt.bar( 10, 41, 0.4, label = 'Dry')
    # plt.bar( 1, 88, 0.4, label = 'Normal')
    #plt.bar(X_axis - 0.2, y[1], 0.4, label = 'Service', color=service_color)

    plt.xticks([2, 4], _type)
    plt.xlabel("Skin Type(SN)")
    plt.ylabel("Classification Score")
    plt.title(f"SN Test Score")
    plt.legend()
    plt.show()    
#skin_classification()
